...
public class LockListener implements MiiListener {
	...
	public void receive(MiiChannel channel, MiiMessage message) {
		if(message.command() == MiiMessage.LOCK){
			...
			log.info("监听到服务端："+channel.name()+"，门锁状态指令: "+ message.command());
			//判断是否为多锁
			if(data.list().size() == 1){
				//单锁处理逻辑
				log.info("监听到服务端："+channel.name()+"，进入单锁处理流程，门锁状态: "+ JSON.toJSONString(data.list()));
			}else{
				//多锁处理逻辑
				log.info("监听到服务端："+channel.name()+"，进入多锁处理流程，门锁状态: "+ JSON.toJSONString(data.list()));
			}
		}
	}
